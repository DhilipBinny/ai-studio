 ...
 * Used by AI Studio's provider management UI.
 */
...
export async function testProviderConnection(provider: ProviderTestConfig): Promise<TestResult> {
...
async function testAnthropic(provider: ProviderTestConfig, start: number): Promise<TestResult> {
  ...
  const timeout = setTimeout(() => {}, TEST_TIMEOUT_MS);
  ...
  try {
    ...
    if (err.status === 429) {
      return { success: true, latencyMs, note: 'Rate limited — auth is valid', models: [] };
    }
    if (err.status === 401) {
      return { success: false, latencyMs, error: isOAuth ? 'Invalid or expired OAuth token (401)' : 'Invalid API key (401)', models: [] };
    }
    if (err.status === 403) {
      return { success: false, latencyMs, error: 'Access denied (403)', models: [] };
    }
    if (err.name === 'AbortError') {
      return { success: false, latencyMs, error: 'Connection timed out', models: [] };
    }
...
async function testOpenAI(provider: ProviderTestConfig, start: number): Promise<TestResult> {
  ...
  try {
    ...
    if (err.status === 401) {
      return { success: false, latencyMs, error: 'Invalid API key (401)', models: [] };
    }
...
async function testOllama(provider: ProviderTestConfig, start: number): Promise<TestResult> {
  ...
  try {
    ...
    if (!res.ok) {
      return { success: false, latencyMs, error: `Ollama returned ${res.status}`, models: [] };
    }
    ...
    if ((e as Error).message?.includes('ECONNREFUSED')) {
      return { success: false, latencyMs, error: `Cannot connect to Ollama at ${baseUrl} — is it running?`, models: [] };
    }
...
async function testOpenAICompatible(provider: ProviderTestConfig, start: number): Promise<TestResult> {
  if (!provider.baseUrl) {
    return { success: false, latencyMs: 0, error: 'Base URL is required', models: [] };
  }

  const client = new OpenAI({
    apiKey: provider.apiKeyRef || 'not-needed',
