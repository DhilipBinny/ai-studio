...
export async function sendEmail(params: {
  ...
  await t.sendMail({
    from: process.env.SMTP_FROM || "alerts@echoltech.com",
    to: params.to,
...
export function buildOTPEmail(code: string): { subject: string; html: string } {
  ...
  return {
    subject: "Your verification code - Echol AI Studio",
    html: `
...
export function buildPasswordResetEmail(resetUrl: string): {
  ...
  return {
    subject: "Password reset - Echol AI Studio",
    html: `
      <div style="font-family: sans-serif; max-width: 400px; margin: 0 auto; padding: 32px;">
        <h2 style="color: #811a1b;">Echol AI Studio</h2>
        <p>A password reset was requested for your account.</p>
        <a href="${escapeHtml(resetUrl)}" style="display: inline-block; padding: 12px 24px; background: #811a1b; color: white; text-decoration: none; border-radius: 6px; margin: 16px 0;">
          Reset Password
        </a>
